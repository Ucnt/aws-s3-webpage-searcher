...
def get_urls(args, rerun):
    ...
    if args.webpage_list:
        ...
        logger.log.critical("Removing %s previously run sites from the %s potential results.  Give me a sec..." %(len(searched_sites), len(domains)))
        return list(set(domains) - set(searched_sites))
